...
BEGIN
    BEGIN TRY
        ...
        INSERT INTO Employee (first_name, last_name, national_id, date_of_birth, country_of_birth, phone, email, address, 
                              emergency_contact_name, emergency_contact_phone, relationship, biography, employment_progress,
                              account_status, employment_status, hire_date, is_active, profile_completion, department_id, 
                              position_id, manager_id, contract_id, tax_form_id, salary_type_id, pay_grade)
        VALUES (@FirstName, @LastName, @NationalID, @DateOfBirth, @CountryOfBirth, @Phone, @Email, @Address,
                ...
                @PositionID, @ManagerID, @ContractID, @TaxFormID, @SalaryTypeID, @PayGrade);
...
BEGIN
    BEGIN TRY
        ...
        IF EXISTS (SELECT 1 FROM EmployeeHierarchy WHERE employee_id = @EmployeeID)
            UPDATE EmployeeHierarchy SET manager_id = @NewManagerID WHERE employee_id = @EmployeeID;
...
BEGIN
    ...
    DECLARE @XML XML = CAST('<i>' + REPLACE(@AffectedEmployees, ',', '</i><i>') + '</i>' AS XML);
    
    INSERT INTO Employee_Notification (employee_id, notification_id, delivery_status, delivered_at)
    SELECT CAST(T.c.value('.', 'VARCHAR(10)') AS INT), @NotificationID, 'Delivered', GETDATE()
    FROM @XML.nodes('/i') T(c)
    WHERE T.c.value('.', 'VARCHAR(10)') <> '';
...
AS
BEGIN
    UPDATE ShiftAssignment SET status = @Status WHERE assignment_id = @ShiftAssignmentID;
...
BEGIN
    DECLARE @ShiftID INT;
    SELECT TOP 1 @ShiftID = shift_id FROM ShiftAssignment WHERE employee_id = @EmployeeID AND status = 'Active';
    
    IF @Type = 'IN'
        INSERT INTO Attendance (employee_id, shift_id, entry_time, login_method)
        VALUES (@EmployeeID, @ShiftID, @ClockTime, 'Offline Sync');
    ELSE IF @Type = 'OUT'
        UPDATE Attendance SET exit_time = @ClockTime, logout_method = 'Offline Sync',
            duration = DATEDIFF(MINUTE, entry_time, @ClockTime)
        WHERE employee_id = @EmployeeID AND CAST(entry_time AS DATE) = CAST(@ClockTime AS DATE) AND exit_time IS NULL;
...
BEGIN
    ...
    ELSE
        SELECT 'Invalid action specified' AS Message;
END;
GO
...
BEGIN
    BEGIN TRY
        ...
        UPDATE Employee SET contract_id = @ContractID WHERE employee_id = @EmployeeID;
...
AS
BEGIN
    UPDATE Contract SET end_date = @NewEndDate, current_state = 'Renewed' WHERE contract_id = @ContractID;
...
BEGIN
    BEGIN TRY
        ...
        FROM LeaveRequest WHERE request_id = @LeaveRequestID;
        ...
        UPDATE LeaveRequest 
        SET status = @Status, approval_timing = GETDATE() 
        WHERE request_id = @LeaveRequestID;
        
        INSERT INTO Notification (message_content, urgency, notification_type)
        VALUES ('Your leave request has been ' + @Status, 'Normal', 'Leave');
...
BEGIN
    ...
    SET current_status = @Decision, approval_date = GETDATE() 
...
BEGIN
    UPDATE LeavePolicy 
    SET eligibility_rules = @EligibilityRules, notice_period = @NoticePeriod 
...
BEGIN
    BEGIN TRY
        IF @FirstName IS NULL OR @LastName IS NULL OR @Email IS NULL OR @HireDate IS NULL
        BEGIN
            SELECT 'Required fields cannot be null' AS Message;
            RETURN;
        END
        ...
        INSERT INTO Employee (first_name, last_name, national_id, date_of_birth, country_of_birth, email, phone, department_id, hire_date)
        VALUES (@FirstName, @LastName, @NationalID, @DateOfBirth, @CountryOfBirth, @Email, @Phone, @DepartmentID, @HireDate);
        ...
        COMMIT TRANSACTION;
        SELECT 'Employee profile created with ID: ' + CAST(@EmployeeID AS VARCHAR) AS Message, @EmployeeID AS EmployeeID;
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
        SELECT 'Error: ' + ERROR_MESSAGE() AS Message;
...
BEGIN
    ...
    SET @SQL = 'UPDATE Employee SET ' + QUOTENAME(@FieldName) + ' = @Value WHERE employee_id = @EmpID';
    
    EXEC sp_executesql @SQL, N'@Value VARCHAR(255), @EmpID INT', @NewValue, @EmployeeID;
...
BEGIN
    DECLARE @SQL NVARCHAR(MAX);
    SET @SQL = 'SELECT * FROM Employee WHERE ' + QUOTENAME(@FilterField) + ' = @Value';
...
BEGIN
    BEGIN TRY
        IF @Name IS NULL OR @Type IS NULL OR @Start_Time IS NULL OR @End_Time IS NULL
        BEGIN
            SELECT 'Required fields cannot be null' AS Message;
            RETURN;
        END
...
BEGIN
    BEGIN TRY
        ...
        SELECT shift_id FROM ShiftCycleAssignment WHERE cycle_id = @ShiftCycle ORDER BY order_number;
        
        OPEN cycle_cursor;
        FETCH NEXT FROM cycle_cursor INTO @ShiftID;
...
BEGIN
    INSERT INTO Notification (message_content, urgency, notification_type)
    VALUES ('Your shift assignment expires on ' + CAST(@ExpiryDate AS VARCHAR), 'Normal', 'Shift');
...
AS
BEGIN
    SELECT 'Permission limits set: Min=' + CAST(@MinHours AS VARCHAR) + ', Max=' + CAST(@MaxHours AS VARCHAR) AS Message;
END;
GO
...
BEGIN
    BEGIN TRY
        ...
        ELSE
            INSERT INTO LeaveEntitlement (employee_id, leave_type_id, entitlement) VALUES (@EmployeeID, @LeaveID, @Entitlement);
...
AS
BEGIN
    UPDATE LeaveRequest SET status = 'Approved', approval_timing = GETDATE() WHERE request_id = @LeaveRequestID;
...
AS
BEGIN
    DECLARE @XML XML = CAST('<i>' + REPLACE(@LeaveRequestIDs, ',', '</i><i>') + '</i>' AS XML);
    
    UPDATE LeaveRequest 
    SET status = 'Approved', approval_timing = GETDATE()
    WHERE request_id IN (SELECT CAST(T.c.value('.', 'VARCHAR(10)') AS INT) FROM @XML.nodes('/i') T(c));
...
BEGIN
    BEGIN TRY
        DECLARE @EmployeeID INT, @LeaveID INT, @Duration INT, @Status VARCHAR(50);
        
        SELECT @EmployeeID = employee_id, @LeaveID = leave_id, @Duration = duration, @Status = status
        FROM LeaveRequest WHERE request_id = @LeaveRequestID;
...
BEGIN
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM Exception WHERE category = 'Leave' AND date BETWEEN @LeaveStartDate AND @LeaveEndDate)
        BEGIN
            BEGIN TRANSACTION;
            ...
            BEGIN
                ...
                VALUES ('Leave Exception - Request #' + CAST(@LeaveRequestID AS VARCHAR), 'Leave', @CurrentDate, 'Active');
                ...
                IF EXISTS (SELECT 1 FROM Attendance WHERE employee_id = @EmployeeID AND CAST(entry_time AS DATE) = @CurrentDate)
                BEGIN
                    UPDATE Attendance 
                    SET exception_id = @ExceptionID
                    WHERE employee_id = @EmployeeID AND CAST(entry_time AS DATE) = @CurrentDate;
                END
...
BEGIN
    BEGIN TRY
        ...
        VALUES (@PayrollID, @EmployeeID, @Type, @Amount, 'USD', CAST(@Duration AS VARCHAR) + ' mins', @Timezone);
        
        INSERT INTO Payroll_Log (payroll_id, actor, modification_type)
        VALUES (@PayrollID, @EmployeeID, 'Payroll item adjusted: ' + @Type + ' = ' + CAST(@Amount AS VARCHAR));
...
BEGIN
    BEGIN TRY
        ...
        BEGIN
            ...
            INSERT INTO Payroll_Log (payroll_id, actor, modification_type)
            VALUES (@PayrollID, @EmployeeID, 'Policy Applied: ' + @Type + ' - ' + @Description);
...
BEGIN
    BEGIN TRY
        ...
        END
        ELSE IF @Eligibility_criteria LIKE '%90%' OR @Eligibility_criteria LIKE '%attendance%'
        BEGIN
            SELECT 
                e.employee_id, 
                e.full_name, 
            ...
            HAVING COUNT(a.attendance_id) >= (DATEDIFF(DAY, DATEADD(MONTH, -3, GETDATE()), GETDATE()) * 0.9);
        END
        ELSE
        BEGIN
            SELECT 
...
BEGIN
    ...
    AND p.period_start >= @StartDate AND p.period_end <= @EndDate
...
AS
BEGIN
    SELECT e.employee_id, e.full_name, COUNT(a.attendance_id) AS missed_punches
    FROM Employee e
    LEFT JOIN Attendance a ON e.employee_id = a.employee_id
...
AS
BEGIN
    IF NOT EXISTS (SELECT 1 FROM PayGrade WHERE grade_name = @GradeName)
    BEGIN
...
AS
BEGIN
    SELECT 'Signing bonus configured: ' + CAST(@BonusAmount AS VARCHAR) + ' for employee ' + CAST(@EmployeeID AS VARCHAR) AS Message;
END;
GO
...
BEGIN
    ...
    VALUES (@InsuranceType, @EmployeeContribution, 'Salary Range: ' + CAST(@MinSalary AS VARCHAR) + ' - ' + CAST(@MaxSalary AS VARCHAR));
...
BEGIN
    ...
    VALUES (@PolicyType, @PolicyDetails, GETDATE());
...
BEGIN
    INSERT INTO PayrollPolicy (type, description, effective_date)
    VALUES ('Overtime', @DayType + ' overtime at ' + CAST(@Multiplier AS VARCHAR) + 'x rate', GETDATE());
    ...
    VALUES (@PolicyID, @Multiplier, @Multiplier * 1.5, 40);
...
BEGIN
    INSERT INTO Payroll_Log (payroll_id, actor, modification_type)
    VALUES (@PayrollRunID, @ModifiedBy, 'Modified ' + @FieldName + ' to ' + CAST(@NewValue AS VARCHAR));
...
BEGIN
    BEGIN TRY
        ...
        UPDATE LeaveRequest 
        SET status = @Decision, approval_timing = GETDATE()
        WHERE request_id = @LeaveRequestID;
...
BEGIN
    ...
    FROM Department d
    LEFT JOIN Employee e ON d.department_id = e.department_id
    LEFT JOIN Mission m ON e.employee_id = m.employee_id AND m.status = 'Approved'
...
BEGIN
    ...
    VALUES (@EmployeeID, @ShiftID, @EntryTime, @ExitTime, @Duration, 'Manual', 'Manual');
...
BEGIN
    UPDATE LeaveRequest 
    SET status = 'Rejected', approval_timing = GETDATE()
    WHERE request_id = @LeaveRequestID;
...
AS
BEGIN
    SELECT 'Leave approval delegated from ' + CAST(@ManagerID AS VARCHAR) + ' to ' + CAST(@DelegateID AS VARCHAR) + ' for period ' + CAST(@StartDate AS VARCHAR) + ' to ' + CAST(@EndDate AS VARCHAR) AS Message;
END;
GO
...
AS
BEGIN
    SELECT 'Irregular leave pattern flagged for employee ' + CAST(@EmployeeID AS VARCHAR) + ': ' + @PatternDescription AS Message;
END;
GO
...
BEGIN
    BEGIN TRY
        ...
        IF EXISTS (
            ...
            AND ((@StartDate BETWEEN CAST(request_id AS DATE) AND DATEADD(DAY, duration-1, CAST(request_id AS DATE)))
                OR (@EndDate BETWEEN CAST(request_id AS DATE) AND DATEADD(DAY, duration-1, CAST(request_id AS DATE))))
        )
        BEGIN
            SELECT 'Leave request overlaps with existing request' AS Message;
            RETURN;
        END
...
BEGIN
    BEGIN TRY
        ...
        IF EXISTS (SELECT 1 FROM Attendance WHERE employee_id = @EmployeeID AND CAST(entry_time AS DATE) = @TodayDate)
        BEGIN
            UPDATE Attendance 
            SET exit_time = @ExitDateTime,
                duration = DATEDIFF(MINUTE, entry_time, @ExitDateTime),
                logout_method = 'Self-Service'
            WHERE employee_id = @EmployeeID AND CAST(entry_time AS DATE) = @TodayDate;
        END
        ELSE
        BEGIN
            INSERT INTO Attendance (employee_id, shift_id, entry_time, login_method)
            VALUES (@EmployeeID, @ShiftID, @EntryDateTime, 'Self-Service');
        END
...
BEGIN
    ...
    VALUES (@ExpenseType, @ExpenseType, 'Pending', @EmployeeID);
...
AS
BEGIN
    SELECT * FROM Mission WHERE employee_id = @EmployeeID ORDER BY start_date DESC;
END;
GO
...
BEGIN
    BEGIN TRY
        ...
        IF @NotificationType = 'Profile Update'
            SET @MessageContent = 'Your profile has been updated successfully';
        ELSE IF @NotificationType = 'Document Upload'
            SET @MessageContent = 'A new document has been uploaded to your profile';
        ELSE IF @NotificationType = 'Document Change'
            SET @MessageContent = 'One of your documents has been modified';
        ELSE IF @NotificationType = 'Verification Update'
            SET @MessageContent = 'Your verification/certification has been updated';
        ELSE IF @NotificationType = 'Contract Update'
            SET @MessageContent = 'Your employment contract has been updated';
        ELSE
            SET @MessageContent = 'Profile update: ' + @NotificationType;
...
BEGIN
    ...
    SELECT 'Flexible attendance logged: ' + CAST(@TotalHours AS VARCHAR) + ' hours' AS Message, @TotalHours AS TotalWorkingHours;
END;
GO
...
BEGIN
    INSERT INTO Notification (message_content, urgency, notification_type)
    VALUES ('You have a missed punch on ' + CAST(@Date AS VARCHAR) + '. Please submit a correction request.', 'High', 'Attendance');
...
BEGIN
    ...
    IF @Type = 'IN'
        INSERT INTO Attendance (employee_id, shift_id, entry_time, login_method)
        VALUES (@EmployeeID, @ShiftID, @ClockInOutTime, 'Self-Service');
    ELSE IF @Type = 'OUT'
        UPDATE Attendance 
...
BEGIN
    ...
    UPDATE LeaveRequest 
    SET duration = @Duration, justification = @Reason
...
AS
BEGIN
    UPDATE LeaveRequest SET status = 'Cancelled' WHERE request_id = @LeaveRequestID AND status IN ('Pending', 'Approved');
...
BEGIN
    ...
    INSERT INTO LeaveRequest (employee_id, leave_id, justification, duration, status)
    VALUES (@EmployeeID, @LeaveTypeID, 'Retroactive: ' + @Reason, @Duration, 'Pending');
...
BEGIN
    INSERT INTO Notification (message_content, urgency, notification_type)
    VALUES ('Your leave request #' + CAST(@RequestID AS VARCHAR) + ' has been ' + @Status, 'Normal', 'Leave');
